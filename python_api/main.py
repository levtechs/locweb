...
def commit_and_push_changes():
    try:
        ...
        # Get list of newly added businesses
        result = subprocess.run(
            ["git", "diff", "--cached", "--name-only", "--", businesses_dir],
            cwd=repo_dir,
            capture_output=True,
            text=True
        )
        ...
        new_businesses = []
        for f in new_files:
            if f.startswith("public/businesses/") and f.endswith("/data.json"):
                slug = f.split("/")[-2]
                name = unquote(slug).replace("-", " ").replace("+", " ")
                new_businesses.append(name)
        
        # Get total count
        total_count = get_current_count()
        
        # Format business names for commit message
        if new_businesses:
            businesses_list = ", ".join(new_businesses[:5])
            if len(new_businesses) > 5:
                businesses_list += f" and {len(new_businesses) - 5} more"
        else:
            businesses_list = "various updates"
        ...
        commit_message = f"""Automated commit: Generated {len(new_businesses)} new website(s)

Total websites: {total_count}

New additions: {businesses_list}
...
This is an automated commit generated by the LocWeb curation script."""
        ...
        if result.returncode == 0:
            print(f"\nCommitted: {len(new_businesses)} new website(s) - {businesses_list}")
